...
# Workaround for PyTorch 2.6+ weights-only loading restrictions.
# Monkey-patch torch.load to use weights_only=False before Stanza imports it.
# This allows Stanza's pretrained models (which use numpy pickles) to load correctly.
# Only do this if you trust the Stanza model sources (official Stanza models are trusted).
import torch
_original_torch_load = torch.load
def _patched_torch_load(f, map_location=None, pickle_module=None, *, weights_only=None, **kwargs):
    # Force weights_only=False to allow loading numpy-based pickles in Stanza models
...
# Download stanza models into local folder if missing (first run can be slow)
...
def reorder_eng_to_isl(input_string):
	# Temporarily disable Stanford Parser download to fix animation issues
	# download_required_packages();
	
	# For now, just return the input in original order
	# TODO: Re-enable Stanford Parser once download is fixed
	return input_string;
...
if __name__=="__main__":
    # Run on port 5001 for text-to-sign Flask backend (frontend expects port 5001)
    print("=" * 70)
    print("🚀 Starting Flask Text-to-Sign Backend")
    print("=" * 70)
